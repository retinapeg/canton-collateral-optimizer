...
class WalletCli:
    ...
    def __init__(
        ...
        *,
        ...
        run_demo_timeout_seconds: float = 300.0,
        ...
        self.run_demo_timeout_seconds = run_demo_timeout_seconds
        self.fixture = (
    ...
    def run_json(
        ...
        if completed.returncode != 0:
            if command == "charge" and _is_expected_attack_rejection(response, argv):
    ...
    def timeout_for(self, command: str) -> float:
        """Keep quick commands bounded without applying that cap to run-demo."""
        return (
            self.run_demo_timeout_seconds
            if command == "run-demo"
        ...
        *,
        ...
        try:
            ...
            raise DemoClientError(
                ...
                f"{self.timeout_for(command):g} seconds.",
...
def _is_expected_attack_rejection(
    response: dict[str, Any], arguments: Sequence[str]
) -> bool:
    """Recognize only definitive Canton evidence for the three fixed attacks."""
    ...
    if http_status in {401, 403}:
        return False
    counterparty = _option_value(arguments, "--counterparty")
    amount = _option_value(arguments, "--amount")
    assertion = error.get("assertion")
    if counterparty == "Merchant-A" and amount == "80":
        return assertion == "charge would exceed the cap"
    if counterparty == "Merchant-B" and amount == "10":
        return assertion in {
            "counterparty is not allow-listed",
            "merchant is not allow-listed",
        }
    if counterparty != "Merchant-A" or amount != "1":
        return False
    ...
    return (
        ...
        and ledger_code == "CONTRACT_NOT_FOUND"
